...
# 9 Jult 2016 - cleaning for SHINY APP
...
processLatLonTree_oct2015 <- function(trLine, youngestTip=youngestTip, translateTbl=translateTbl ) {
	#trLine   <- lines[i]
	els	   <- strsplit(trLine, "\\[\\&R\\] ")[[1]]
	tempTr   <- els[2]
...
read_BEAST_trees_LatLon	<- function( treesName, nsamples=-1, burnin=0, step=1 ) {
	...
	trs		<- lapply(lines, processLatLonTree_oct2015, youngestTip=youngestTip, translateTbl=translateTbl)
